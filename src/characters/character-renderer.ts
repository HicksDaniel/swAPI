  ...
  const characterHTMLEntries = characters.map(character => {
    return `<div class="characters">
      ...
      <h4>HomeWorld: ${character.homeworldName}</h4>
        <ul>
          <li>Films:</li>
          ${character.filmNames.map((filmName) => `<li>${filmName}</li>`).join("")}
        </ul>
   </div>`
  })
...
const fetchCharacterData = async () => {
  ...
  const characterWithData: CharacterData[] =[];

  const characterDataResponse = await jsonFetch<PaginatedResponse<CharacterResponse>>(`${BASE_URL}/people`)

  for (const character of characterDataResponse.results) {
    const filmPromises = character.films.map((filmUrl: string) => jsonFetch<FilmData>(filmUrl));
